...
LLM = os.environ.get('OPENAI_MODEL_NAME', os.environ.get('LLM_FALLBACK_MODEL', 'gemini-2.5-pro'))  # Use configurable fallback
...
class CustomGraphProcessor:
    def __init__(self, model_config=None):
        ...
        self.driver = neo4j.GraphDatabase.driver(NEO4J_URI, auth=(NEO4J_USER, NEO4J_PASSWORD), database=CLIENT_NEO4J_DATABASE)
        ...
        logger.debug(f"Embedding expected dim: {expected_dims}")
        ...
        self.gds = GraphDataScience(NEO4J_URI, auth=(NEO4J_USER, NEO4J_PASSWORD), database=CLIENT_NEO4J_DATABASE)
        ...
        with self.driver.session() as session:
            ...
            for constraint in constraints:
                try:
                    ...
                    import traceback
                    logger.error(f"Unable to create constraints: {traceback.print_exc()}. Exception: {e}")
            ...
            # Create unified vector index for all entities #added "s" entity_embedding"s"
    ...
    def _process_document_text(self, text: str, doc_name: str, source_info: str) -> Dict[str, Any]:
        ...
        with self.driver.session() as session:
            ...
            """, doc_id=doc_id, doc_name=doc_name, source_info=source_info, 
                text=text, embedding=doc_embedding, chunk_count=len(chunks))
